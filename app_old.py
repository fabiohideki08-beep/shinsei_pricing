...
app = FastAPI(title="Shinsei Pricing V4")
BASE_DIR = Path(__file__).resolve().parent
DATA_DIR = BASE_DIR / "data"
DATA_DIR.mkdir(exist_ok=True)


def _choose_data_path(filename: str) -> Path:
...
REGRAS_PATH = _choose_data_path("regras.json")
CFG_PATH = _choose_data_path("integracao_config.json")
APR_PATH = _choose_data_path("aprovacoes.json")
HIST_PATH = _choose_data_path("historico.json")
PEND_PATH = _choose_data_path("pendentes.json")
...
def home():
    for nome in ("index_v4.html", "index_v3.html", "index.html"):
        index_path = BASE_DIR / nome
        if index_path.exists():
            return HTMLResponse(index_path.read_text(encoding="utf-8"))
    return HTMLResponse("<h2>Shinsei Pricing V4</h2><p>index_v4.html não encontrado.</p>")
...
def calcular(payload: SimulacaoPayload):
    ...
    _append_jsonl(PRICING_LOG_PATH, {"tipo": "simulacao", "data": _now_iso(), "sku": payload.sku, "resultado": resultado})
    return resultado
...
def bling_callback(code: str | None = None, error: str | None = None):
    ...
    return HTMLResponse(
        f"<html><body style='font-family:Arial;padding:30px'><h2>Conexão com Bling realizada</h2><p>Tokens salvos com sucesso.</p><pre>{json.dumps({'scope': tokens.get('scope'), 'expires_in': tokens.get('expires_in')}, ensure_ascii=False, indent=2)}</pre><p><a href='/'>Voltar ao sistema</a></p></body></html>"
    )
...
def bling_status():
    ...
    return {
        ...
        "arquivo_tokens": str(BASE_DIR / "data" / "bling_tokens.json"),
    }
...
def _build_bling_result(payload: PrecificacaoBlingPayload):
    regras = carregar_regras(True)
    if not regras:
        raise HTTPException(400, "Nenhuma regra ativa cadastrada.")
    ...
    preco_atual = float(produto.get("preco") or 0)
    preco_sugerido = float((resultado.get("melhor_resultado") or {}).get("preco_promocional") or 0)
    resultado["comparacao_preco"] = _comparar_variacao(preco_atual, preco_sugerido, cfg)
    resultado.setdefault("auditoria", {})
    resultado["auditoria"].update({
        "formula_version": (resultado.get("auditoria") or {}).get("formula_version", "v2.1.0"),
        "historico_path": str(HIST_PATH),
...
def bling_pre_visualizar(payload: PrecificacaoBlingPayload):
    resultado = _build_bling_result(payload)
    resultado["preview_only"] = True
    resultado["fila_aprovacao"] = {"criado": False, "status": "pre_visualizacao"}
    _append_jsonl(PRICING_LOG_PATH, {"tipo": "bling_preview", "data": _now_iso(), "valor_busca": payload.valor_busca, "resultado": resultado})
    return resultado
...
def bling_precificar(payload: PrecificacaoBlingPayload):
    ...
    if payload.modo_aprovacao == "manual" and "erro" not in resultado and resultado.get("melhor_resultado"):
        if resultado["comparacao_preco"]["entrou_na_fila"]:
            item = _criar_item_aprovacao(resultado, "precificacao_manual", payload.valor_busca, "recalculo_manual")
    ...
    elif payload.modo_aprovacao == "automatico" and "erro" not in resultado:
        resultado["fila_aprovacao"] = {"criado": False, "status": "pronto_para_aplicacao_automatica"}

    _append_jsonl(PRICING_LOG_PATH, {"tipo": "bling", "data": _now_iso(), "valor_busca": payload.valor_busca, "resultado": resultado})
    return resultado
...
async def webhook_bling(request: Request):
    try:
        ...
        item = {
            "id": str(uuid4()),
            "produto_nome": payload.sku or payload.ean or payload.produto_id or "Evento Bling",
            "valor_busca": payload.sku or payload.ean or payload.produto_id or "",
            "motivo": payload.motivo,
            "status_aprovacao": "pendente",
            "data_criacao": _now_iso(),
            "origem": "webhook",
            "payload_webhook": payload.payload or data,
        }
        add_aprovacao(item)
        return {"status": "ok", "item": item}
    except Exception as e:
        _append_jsonl(ERROR_LOG_PATH, {"data": _now_iso(), "tipo": "webhook", "erro": str(e)})
